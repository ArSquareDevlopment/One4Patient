...
//  One for Patient
//
//  Created by Idontknow on 11/02/20.
...
private struct ZoomAPI {
  ...
  static let appKey = "6HQ3XhXI4i3fFoZTFW6SQH4Ysl2vo2wVjF11"
  static let appSecret = "NiRDWTGJqjdpS1h3lepAfoPsoMAflfXsPCPB"
  
  // API User info for starting calls as API user.
  static let userID = ""
  static let userToken = ""
//    "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJhdWQiOm51bGwsImlzcyI6IlZMcmpnbzYxU0dHUHdvRzVndGxJSkEiLCJleHAiOjE1ODIxNzMwMDEsImlhdCI6MTU4MTU2ODIwMX0.bGTYwdvnRnWMzNZkl9npyzFIri-G8PQJ48R2cPhoNFA"
//
  // Default display name for meetings.
  static let defaultName = "Gopu"
}
...
extension ZoomService {
  ...
  func login(email: String, password: String) {
    guard isAPIAuthenticated, let authService = MobileRTC.shared().getAuthService() else {  print("No Login"); return }
    authService.login(withEmail: email, password: password, remeberMe: false)
  }
  ...
  func onMobileRTCLoginReturn(_ returnValue: Int) {
    ...
    preMeetingService.delegate = self as? MobileRTCPremeetingDelegate
  ...
  func scheduleMeeting(topic: String, startTime: Date, timeZone: TimeZone = NSTimeZone.local, durationInMinutes: TimeInterval) {
    ...
    preMeetingService.scheduleMeeting(meeting, withScheduleFor: "Meeting")
